...
void main(uint3 DTid : SV_DispatchThreadID)
{
	...
	const uint stride = 60; // 後で定数バッファにする
	const uint fetchAddress = index * stride;

	uint size = 0;
	OutVertices.GetDimensions(size);
...
//	 if (VerticesNum < index) return;
//
//	OutVertices[index].pos.x = OutVertices[index].pos.y = OutVertices[index].pos.z = 0;
//	float4 p = float4(VerticesSource[index].pos.x, VerticesSource[index].pos.y, VerticesSource[index].pos.z, 1.0);
//	float3 n = float3(VerticesSource[index].normal.x, VerticesSource[index].normal.y, VerticesSource[index].normal.z);
//
//	// Skinning
//	for (int i = 0; i < Weights[index].count; ++i)
//	{
//		int b = Weights[index].boneIndex[i];
//
//		OutVertices[index].pos    += mul(KeyMatrices[b], p) * Weights[index].weight[i];
//		OutVertices[index].normal += mul((float3x3)KeyMatrices[b], n) * Weights[index].weight[i];
//	}
//}
